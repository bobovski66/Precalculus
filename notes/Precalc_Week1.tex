...
This week begins with one of the central habits of precalculus: looking at a set of
points and asking whether there is a rule behind them. Sometimes the points are just
a collection of measurements. Sometimes they line up in a pattern clearly enough that
a formula can describe them. The vocabulary of \emph{relations} and \emph{functions}
gives us a way to separate those two situations.

A relation is a collection of points. A function is a relation in which each input
determines a single output. That condition matters because it is the condition that
makes prediction possible: once the input is known, the output is determined. Much of
this course is about finding predictable rules in data, recognizing familiar shapes,
and then reshaping those familiar functions into models of the form
\[
  A\,f\!\big(b(x-h)\big)+k.
\]
The algebra is important, but the guiding question is geometric: what shape are we
looking at, and how has it been moved?

\section{Relations as Point Clouds}

Before we start with formulas, it helps to think about data. Record, for every
student in a class, their height and their shoe size. Each student becomes a pair of
numbers: an input (height) and an output (shoe size). Plotted, these pairs form a
cloud of dots.

At this stage there is no formula yet. The cloud is simply the information we have.
It may suggest a pattern, or it may not. A relation is the mathematical object that
lets us talk about the cloud before deciding whether it deserves to be called a
function.
...
four ways of naming the same set of pairs, and moving among those forms is one of the
basic skills of the course. The list
\[
\{(1,2),\ (3,5),\ (3,-1),\ (4,0)\}
\]
...
Notice that the example did not require a formula. A finite list of points is already
a relation. The formula enters only when we try to describe many points at once, or
when we want to predict values that were not listed.

\section{When a Relation Is a Function}

Now we add the extra condition that makes a relation predictable. A general relation
may pair the same input with several outputs. That is allowed for a relation, but it
is not allowed for a function.

A function is stricter. Once an input has been chosen, the function gives one output
and only one output. This is the reason functions are useful as models: they let us
ask, ``What happens when the input is this?'' and expect a definite answer.
...
The issue is not whether the relation is interesting or geometric. The issue is
whether each input selects a single output.
\begin{keyidea}
...
$f$ multiplied by $2$. This notation is compact, but it is also literal: whatever is
inside the parentheses is the input being fed into the rule.
...
The last part is the one to practice slowly. The input is not just the symbol $a$;
the input is the whole expression $a+1$. Parentheses protect that input while the
function rule acts on it.
...
The function property also has a graphical reading. A graph is made of points, and
points with the same $x$-coordinate lie vertically above one another. So the statement
``each input gives one output'' means that above any point on the $x$-axis the graph
has at most one point. A vertical line is exactly the set of points sharing one
$x$-value, which gives the test directly.
\begin{keyidea}
...
The two ideas now meet. Real data usually forms a point cloud with noise; it is
almost never an exact function. Two people with the same height may have different
shoe sizes, and repeated measurements of the same process may not agree perfectly.
That does not make functions useless. It means we use them carefully.

A function can be laid through the cloud as a \emph{model}: a predictable rule that
follows the trend closely enough to summarize the data and make reasonable forecasts.
The model is not a claim that the data points magically lie on a perfect curve. It is
a controlled simplification, chosen because it captures the main pattern.
\begin{keyidea}
...
climbs quickly and then levels off suggests a square-root or logarithmic shape. This
is why we build a catalog of \emph{parent functions}. They are the basic shapes we
learn to recognize before the algebra becomes complicated.

Once the shape has been chosen, the remaining work is to move and resize it. That is
where transformations enter.
...
So far, a function has been a rule that assigns one output to each input. Now we use
functions as shapes. The point is not only to evaluate a formula, but to understand
how changes in the formula move the graph.
...
\]
Each constant controls one geometric action. When we fit a transformed parent
function to data, we are usually not inventing a new formula from nothing. We choose
the parent shape first, then use $A$, $b$, $h$, and $k$ to move and resize it until it
matches the cloud reasonably well. The constants become geometric controls.
...
$f$ ($A$ and $k$) act on the output, so they move the graph \emph{vertically}. These
usually behave the way the notation suggests: $+k$ raises the graph, and $A$ scales
its height.

Constants applied \emph{inside} $f$ ($b$ and $h$) act on the input before the parent
function sees it. Because they change the input, they move the graph
\emph{horizontally}, and the direction can feel backwards at first: $x-h$ shifts
\emph{right} by $h$, and multiplying the input by $b$ compresses the graph
...
The useful habit is to look for a landmark point on the parent graph, such as a
vertex, corner, endpoint, or intercept. Transform that point first. Once a few
landmarks are in place, the rest of the graph is much easier to sketch.
...
When several transformations combine, the safest method is to follow one point on
the graph. The order mirrors the evaluation of $g(x)$ at a number: work from the
input outward. Apply the inside first (horizontal: the $b$ compression, then the $h$
shift), then the outside (vertical: the $A$ stretch or reflection, then the $k$
shift). Following one landmark point of the parent through each step keeps the order
honest and prevents the horizontal changes from being applied backwards.
...
Fitting is limited by the shapes one knows. The following parent functions recur
throughout the course. The goal is not to memorize them as isolated formulas, but to
recognize their visual behavior: straight, bowl-shaped, V-shaped, S-shaped,
flattening, branching, or rapidly growing. Once the rough shape is recognized, the
transformation parameters can do the more precise fitting.
\begin{center}
...
At this stage, a good first guess is more valuable than a perfect answer. Choose the
parent family from the overall shape, then adjust the constants to improve the fit.
