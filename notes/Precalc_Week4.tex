...
This week takes the polynomial ideas from Week 3 and adds division. A rational
function is a ratio of two polynomials,
\[
f(x)=\frac{p(x)}{q(x)}.
\]
Because both the numerator and denominator can have roots, rational functions have
more visible features than polynomials. Some roots pull the graph down to the
$x$-axis. Other roots make the formula undefined and force the graph toward a
vertical line.

The graph is governed by the interplay between numerator and denominator. Where the
numerator $p$ is zero, the function is zero. Where the denominator $q$ is zero, the
function is undefined and the graph can run off toward a vertical asymptote. The
relative sizes of the two degrees decide what happens at the far ends. As with
polynomials, factored form displays the local features, and leading terms settle the
end behavior.

\section{Zeros and Vertical Asymptotes}

The first features to locate are zeros and vertical asymptotes. Both come from roots,
but they come from different parts of the fraction.
...
The common factor is important because it tells us that the original formula was
undefined at $x=-1$, even though the simplified formula has the same values
elsewhere. Graphically, that leaves a missing point rather than a whole vertical
asymptote.
...
Once zeros and vertical asymptotes are marked, the graph is broken into intervals.
On each interval the function is continuous and keeps one sign. This means the same
sign-chart idea from polynomials still works, but now the feature points include both
...
This chart gives the side of the $x$-axis but not the exact height of the graph. Near
$x=3$, the asymptote supplies the missing information: the curve must escape upward
or downward instead of passing through the vertical line.

\section{End Behavior: Comparing Degrees}

Zeros and vertical asymptotes describe what happens near special finite inputs. End
behavior asks a different question: what does the graph do far away from the origin?
For rational functions, the answer comes from comparing the size of the numerator to
...
The horizontal asymptote is not found by plugging in a convenient value of $x$. It is
a statement about what the graph approaches when $x$ moves far to the left or far to
the right.
...
When the numerator degree is smaller than the denominator degree, the fraction itself
shrinks toward zero at the ends. When the degrees are equal, the ratio of leading
coefficients gives a horizontal asymptote. The remaining case is different: the
numerator is at least as large in degree as the denominator.
...
curve. This is the rational-function version of listening to the dominant part of
the formula at large scale.
...
slant asymptote. The division will separate the function into a line plus a leftover
fraction that becomes small at the ends. Divide $x^2+x-2$ by $x-3$:
\[
\begin{array}{r}
x+4\phantom{xxxxx}\\
...
This example is useful because it separates two ideas that students often connect too
strongly. A rational function can have a slant asymptote without having any vertical
asymptotes. Vertical asymptotes come from real roots of the denominator; slant
asymptotes come from degree comparison and division.

\section{Assembling the Graph}

A rational graph can look complicated because different pieces of information are
happening at once: zeros, holes, vertical asymptotes, end behavior, and signs. The
way to make the sketch manageable is to read these features one at a time.

A faithful sketch of a rational function in lowest terms combines several readings,
each taken from the factored form or the long division.
...
We will read the function in the standard order: reduce first, then locate zeros and
vertical asymptotes, then determine end behavior, and finally use signs to guide the
sketch.
...
The sketch is not produced by guessing the curve all at once. It is assembled from
features. The asymptotes form a frame, the zeros pin the curve to the axis, the
horizontal asymptote controls the outer branches, and the sign information keeps each
piece on the correct side of the axis.
...
Use the lab to practice reading the graph as a set of features rather than as one
large drawing problem. Move one zero or one asymptote at a time and watch which part
of the graph responds.
...
\item Identify the hole, the zeros, and the vertical asymptotes after reducing to lowest terms:
\[
f(x)=\frac{(x-3)(x+2)}{(x+2)(x-5)}.
\]
