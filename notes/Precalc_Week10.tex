...
Last week, we solved systems by writing and rewriting equations. That method is
honest, but the variable names $x,y,z$ do not really change while we work. What moves
are the coefficients. A matrix keeps exactly the information that changes and places
it in a grid where the bookkeeping is easier to control.
...
the column after the bar holds the constants. The vertical bar is not a new operation;
it is only a reminder that the right-hand side has been placed next to the
coefficients.
...
For example, the first column above belongs to $x$, the second to $y$, and the third
to $z$. A zero coefficient would still need to be written as a zero in its column;
otherwise the alignment of the variables would be lost.

\section{Row Operations}

The three reversible moves on equations, from the previous chapter, become the three
elementary row operations. Each leaves the solution set unchanged. The guiding rule is
simple: a row is an equation, so a legal row move must be a legal equation move.
...
This is the same triangular shape as before, but compressed into a table of numbers.
The work is not conceptually different from elimination by hand; the notation has just
...
The method is deliberately mechanical. At each stage, choose a leading entry, use it
to create zeros below it, and then move down and to the right. This regularity is what
makes the method reliable, especially when a system is too large to solve comfortably
by informal substitution.
...
The row operations did not solve the system all at once. They prepared the system so
that the solution could be read from the bottom upward. This is why echelon form is
the real target of the method.
...
The bottom row is often the most revealing row. It either gives the last variable,
exposes a contradiction, or disappears into the identity $0=0$.
...
The applications below use the same method, not a new one. The only new step is the
translation: choose variables, write one linear equation for each constraint, then use
the matrix method to solve or describe the resulting system.
...
\end{aligned}
\]
The augmented matrix and its elimination are
\[
\left[\begin{array}{ccc|c}
...
Balancing a chemical equation is another conservation problem. The number of atoms of
each element must be the same on both sides. Those conservation statements become a
linear system for the coefficients in the balanced equation.
...
Network-flow problems have the same conservation structure. At each intersection,
flow in equals flow out. If the data does not determine every road segment uniquely,
the matrix will reveal that with a free parameter.
