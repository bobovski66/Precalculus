...
In the first two weeks we studied functions by looking for shape: lines have a fixed
rate of change, parabolas have a vertex and symmetry, and transformations move known
parent graphs into new positions. This week extends that same way of thinking to
polynomials of higher degree.

A polynomial of high degree can look intimidating in expanded form, just a long sum
of powers with no obvious shape. The main point of the week is that the graph is not
as mysterious as it first appears. A small amount of information controls the rough
shape: what the graph does at the far left and far right, and what happens at each
root. The factored form displays this information. Each factor names a root, the
exponent on that factor governs whether the graph crosses the axis or turns back
from it, and the leading term governs both ends.
...
Lines are degree-one polynomials and parabolas are degree-two, so this chapter is
not beginning a completely new subject. It is widening a pattern we have already
seen. A polynomial is built from powers of $x$, and the highest power present is its
degree.

The expanded form above is convenient for evaluation and for reading the constant
term $a_0=f(0)$. If someone asks for $f(2)$, expanded form is often efficient. But
expanded form can conceal the shape of the graph. The factored form reveals the
places where the graph meets the $x$-axis, and those places are the anchors for a
sketch.
...
When sketching a polynomial, it is natural to begin with the two ends of the graph.
The ends tell us the large-scale direction of travel before we worry about the
middle.
...
This does not mean the lower-degree terms are irrelevant. They can create turns,
roots, and vertical shifts in the middle of the graph. They simply cannot compete
with the leading term far out on the left and right.
...
End behavior tells us where the graph is going at the two extremes. Roots tell us
where the graph is pinned to the $x$-axis. These are the points where the output is
zero, so they are also the places where the sign of the function may change.
...
of times that factor is repeated. The multiplicities sum to the degree $n$. This
count is useful because it connects the local behavior at the roots to the total
...
Multiplicity controls how the graph meets the axis at the root. The rule is a
parity rule, the same parity idea that governed the ends. Odd powers preserve the
sign of a factor as it passes through zero, while even powers remove the sign change.
That algebraic fact is what becomes crossing or touching in the graph.
...
This one example already contains the basic sketching data: two ends, three roots,
and the crossing-or-touching behavior at each root. A more accurate graph would need
more information, but a faithful rough sketch begins here.
...
The next step is to decide where the graph lies above the $x$-axis and where it lies
below. Roots mark possible changes, but they do not by themselves tell us the sign on
each interval. A sign chart supplies that information.
...
We already know the roots and multiplicities. Now we use the same factored form to
track the sign of the product. The roots $-2,1,3$ divide the line into four
intervals. Evaluate the sign of each factor at a test point in each interval; the
factor $(x-1)^2$ is never negative, so it never affects the sign.
\begin{center}
...
The table may look like bookkeeping, but it is doing geometric work. Each plus sign
places the graph above the axis on an entire interval, and each minus sign places it
below.
...
We can now assemble the sketch. The point is not to plot dozens of values. The point
is to combine the structural information already available from the formula.
...
three pieces of data determine the global shape up to the heights of the turns,
which precalculus does not pin down exactly but calculus does. A faithful sketch
needs only these pieces.
...
We gather the information in the order that makes the graph easiest to draw: ends,
roots, sign chart, then a smooth connection.
...
A sketch like this is not claiming exact turning-point heights. It is claiming the
correct global structure: the right ends, the right intercept behavior, and the right
side of the axis on every interval.
