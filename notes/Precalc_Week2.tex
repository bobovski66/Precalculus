...
The two simplest non-constant functions are the line and the parabola, and each
carries a geometric meaning that the algebra alone hides. A line records a fixed
rate of change: every step to the right raises the output by the same amount, which
is the statement that the right triangles riding along the line are all similar. A
parabola records a focusing property: rays arriving parallel to its axis converge to
a single point.

\section{Lines as Families of Similar Triangles}

Take any two points on a non-vertical line and drop the right triangle between them:
run horizontally, then rise vertically to meet the line. Choose a different pair of
points and build the same triangle. The two triangles have the same angles, because
the line meets every horizontal at one fixed angle, so they are similar. Similar
triangles have proportional sides, which means the ratio of rise to run is identical
no matter which pair of points generated it. That common ratio is the slope.
...
The number $b$ enters as the height of the line where it crosses the $y$-axis, the
...
line and only one. The construction runs in three stages: compute the slope from the
two points, write the point-slope form, then rearrange to slope-intercept.
...
\frac{y-y_1}{x-x_1}=m.
\]
Clearing the denominator gives the point-slope form.
...
horizontal line. Through $(2,1)$ and $(2,7)$ the run is $5-5=0$; the slope formula
divides by zero. The relation $x=2$ is a vertical line, which fails the vertical line
test and so is not a function.
\end{example}

\section{The Parabola as a Focusing Curve}

A parabola is not merely a bent line. It is the curve defined by a focusing property:
every ray traveling parallel to its axis reflects off the curve and passes through a
single point, the focus. This is why headlight reflectors, satellite dishes, and
solar collectors are built as parabolas; the geometry concentrates parallel incoming
energy at one point, or, run in reverse, sends energy from the focus out as a
parallel beam.
...
The focusing property has an equivalent point-and-line description, which is the
formal definition of the parabola.
...
as two points determine a line. The vertex sits halfway between focus and directrix,
and the axis is the perpendicular from the focus to the directrix.
...
\]
which carries three parameters $a$, $b$, $c$. Three points with distinct
$x$-coordinates impose three conditions, and solving the resulting system fixes the
three parameters uniquely. So three points determine the quadratic, in the same
sense that two points determine the line.
...
Substitute each point into $f(x)=ax^2+bx+c$:
\[
\begin{aligned}
(0,1):&\quad c=1,\\
(1,0):&\quad a+b+c=0,\\
...
A quadratic function can be written three ways, each exposing different information.
The forms describe the same parabola; converting among them is routine algebra.
...
In all three the same $a$ controls the opening: upward for $a>0$, downward for
$a<0$, and steeper as $|a|$ grows. The vertex form is the transformation form from
Week 1 with parent $f(x)=x^2$: $h$ shifts horizontally, $k$ shifts vertically, $a$
scales and possibly reflects.
...
perfect square hidden in the first two terms. Factor $a$ from the $x$-terms, add and
subtract the square that completes the bracket, then collect.
...
The factored form exposes the $x$-intercepts, the inputs where $f(x)=0$. They come
from factoring directly when possible, or from the quadratic formula in general:
\[
r_{1,2}=\frac{-b\pm\sqrt{b^2-4ac}}{2a}.
\]
The quantity under the root, $b^2-4ac$, is the \emph{discriminant}. It records how
many real roots exist: two when positive, one (a doubled root at the vertex) when
...
The three forms share structure, and a few relations connect their parameters
directly.
